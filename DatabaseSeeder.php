...
if (mysqli_num_rows($connection->query("SELECT * FROM musorok")) > 0) {
  $connection->query("TRUNCATE TABLE musorok");
}
...
foreach ($content->channels as $channel) {
  foreach ($channel->programs as $program) {
    ...
    $connection->query("INSERT INTO musorok (csatorna_neve,	musor_kezdete,	musor_cime,	rovid_leiras,	korhatar, datum) 
