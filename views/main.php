          ...
          <div class="d-flex justify-content-between my-3 align-items-center">
            <div>
              ...
              <?php while ($row = $this->query_channels->fetch_assoc()) : ?>
                <button class="btn <?php $this->channel_name == $row['csatorna_neve'] ? print 'btn-danger' : '' ?>" 
                 type="submit" name="channel_name" value="<?= $row['csatorna_neve'] ?>"><?= $row['csatorna_neve'] ?></button>
              <?php endwhile; ?>
            </div>
            <div>
            ...
            <div>
              ...
              <input class="form-control" id="date" type="date" name="date" 
                min="<?= date('Y-m-d', time() - 3600 * 24 * 2); ?>" 
                max="<?= date('Y-m-d', time() + 3600 * 24 * 11); ?>" 
                value="<?php $this->result_date == date('Y-m-d') ? print date('Y-m-d') : print $this->result_date ?>">
            </div>
          </div>
        </form>
